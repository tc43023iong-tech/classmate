...
import { X, Save, Download, Globe, RefreshCw, Key, Info, CheckCircle2 } from 'lucide-react';
...
const SyncModal: React.FC<SyncModalProps> = ({ isOpen, onClose, trainerCode, onSave, onLoad }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: `Class Saved to Cloud! Code: ${code}` });
    } catch (e) {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Class Loaded Successfully!' });
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 font-pixel">
      <div className="bg-slate-100 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden border-8 border-slate-400 relative">
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="bg-white p-6 rounded-lg border-4 border-slate-300 shadow-inner">
             <div className="flex justify-between items-start mb-4">
                <div>
                   <h4 className="text-2xl font-black text-slate-800 uppercase tracking-wide">Back up to Cloud</h4>
                   <p className="text-slate-500 text-lg">Upload your current class to access it anywhere.</p>
                </div>
                <button 
                  onClick={handleSave}
                  disabled={isSyncing}
             ...
             {trainerCode && (
               <div className="bg-blue-50 p-4 rounded border-2 border-blue-200 flex flex-col items-center">
                  <span className="text-blue-400 text-sm font-bold uppercase">Your Trainer Code</span>
                  <span className="text-5xl font-black text-blue-700 tracking-[10px] my-2">{trainerCode}</span>
                  <p className="text-blue-500 text-center">Share this code to load your class on another device!</p>
               </div>
             )}
          </div>
          ...
          <div className="bg-slate-800 p-6 rounded-lg border-4 border-slate-900 shadow-xl text-white">
             <h4 className="text-2xl font-black uppercase tracking-wide mb-4 text-blue-400">Restore from Cloud</h4>
             ...
             <p className="mt-3 text-slate-400 flex items-center gap-2">
                <Info size={16} /> Warning: Loading will overwrite your local class data!
             </p>
          </div>
        ...
        {/* Footer */}
        <div className="p-4 bg-slate-200 border-t-4 border-slate-400 text-center text-slate-600 font-bold uppercase tracking-widest">
           Professor Oak's Remote PC System v1.0
        </div>
